...
- `queued` when an HTTP `/sms` or `/ussd` request is accepted into the MySQL queue.
- `sent` or `failed` for SMS queue results.
- `done` or `failed` for USSD queue results.
...
| `-version` | Print repository URL, copyright/version and tagline, then exit. |
