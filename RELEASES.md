...
- `v0.5.0` - актуальный релиз: команды `status`/`reset` через очередь и HTTP (ответ во входящие + webhook), описания кодов ошибок (`error_desc`/`state_desc`), надёжность при остановке и ротации лога, валидация очереди до захвата линии.
- `v0.4.0` - webhook-мониторинг линий, события отправки в любом режиме, запрет редиректов webhook, `-update`, `check_updates`, `clear_logs_on_start`, баннер-рамка.
