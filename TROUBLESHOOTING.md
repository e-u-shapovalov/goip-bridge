...
## Проблема: webhook не доставляется, в логе `webhook WARN 301`

Причина: `webhook_url` ведёт на адрес, который отвечает редиректом (`301`/`302`) - например `http://` -> `https://` или `link.kg` -> `www.link.kg`. Bridge редиректы не выполняет: следование за редиректом по стандарту превращает `POST` в `GET` и **отбрасывает тело запроса**, поэтому ответ `3xx` считается ошибкой доставки. В логе видна строка вида:

```text
webhook WARN 301: expected 200, got 301 — webhook_url redirects, the POST body is dropped, fix the URL, Location: https://www.example.com/goip-webhook
```

Событие при этом не теряется: bridge повторяет доставку по `webhook_retry`, пока вы не почините URL (или пока не истечёт `max_hours`).

В версиях до v0.4.0 bridge молча шёл за редиректом - до приёмника доходил пустой `GET`, а доставка считалась успешной. Если видите такое поведение, обновитесь.

Решение: укажите в `webhook_url` конечный адрес без редиректа. Проверьте кандидата:

```sh
curl -I https://example.com/goip-webhook
```

Если ответ `301`/`302` - возьмите адрес из заголовка `Location` (или почините редирект на стороне сайта) и пропишите именно его. Признак исправления: `curl -I` отвечает `200` без смены схемы/хоста, а в логе bridge на каждую доставку - `webhook OK 200`.
