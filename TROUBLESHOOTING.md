...
## Проблема: webhook приходит, но пустой или как `GET`

Принимающий сервис видит запрос (`User-Agent: Go-http-client`), но метод - `GET`, а тело пустое, хотя bridge всегда шлёт `POST` с JSON.

Причина: `webhook_url` ведёт на адрес, который отвечает редиректом (`301`/`302`) - например `http://` -> `https://` или `link.kg` -> `www.link.kg`. HTTP-клиент Go следует за редиректом, но по стандарту превращает `POST` в `GET` и **отбрасывает тело запроса**. До приёмника доходит пустой `GET`.
...
Если ответ `301`/`302` - возьмите адрес из заголовка `Location` (или почините редирект на стороне сайта) и пропишите именно его. Признак исправления: `curl -I` отвечает `200` без смены схемы/хоста.
