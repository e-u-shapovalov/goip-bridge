...
2. Откройте последний релиз, сейчас это **v0.4.0**.
...
Скачивайте asset `goip-bridge` из последнего релиза (сейчас `v0.4.0`). Не скачивайте `Source code` и не используйте `Code -> Download ZIP`.
...
Опубликованы релизы `v0.1.0`, `v0.2.0`, `v0.3.0`, `v0.3.1` и `v0.3.2`. Последний релиз `v0.4.0` содержит готовый asset `goip-bridge` размером около 11 MB для Linux x86-64 / amd64.
