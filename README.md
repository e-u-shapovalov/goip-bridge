...
Ответ придет прямо в curl. Если задан `webhook_url`, результат продублируется webhook-событием `type:"done"` в любом режиме - и без MySQL, и с очередью; в MySQL-режиме перед ним придёт ещё `type:"queued"`.
...
При обновлении меняется только бинарник, `config.json` остаётся на месте.

Самый короткий путь - встроенное самообновление. Команда скачивает последний релиз и `checksums.txt`, сверяет SHA256, делает резервную копию `.bak`, атомарно подменяет бинарник и удаляет `.bak` при успехе (`.bak` остаётся только если обновление сорвалось - для отката):
...
Если запустить `-update` от root, перезапуск systemd-сервиса выполнится автоматически.

Ручной способ - скачайте новую версию рядом, атомарно подмените и перезапустите сервис:
...
Узнавать о новых версиях можно автоматически: включите в `config.json` опцию `"check_updates": true` - при старте bridge фоново спросит GitHub и, если вышла новая версия, напечатает заметную рамку с её номером. По умолчанию проверка выключена (bridge никуда не «звонит»), и в логе об этом одна строка.
...
2. Откройте последний релиз, сейчас это **v0.5.0**.
...
- Webhook-события результатов отправки (`queued`, `sent`, `done`, `failed`) в любом режиме - с MySQL-очередью и без неё.
- Webhook-мониторинг линий: `line_down`/`line_up` при пропадании и восстановлении keepalive, `line_failing`/`line_recovered` при серии ошибок отправки (порог `fail_threshold`).
- Надёжная доставка webhook: очередь в памяти, retry с exponential backoff, запись недоставленных событий в fallback-журнал. Редиректы не выполняются (3xx = ошибка доставки), HTTP-статус каждой доставки виден в логе.
- Самообновление одной командой: `goip-bridge -update` (с проверкой SHA256 и откатом через `.bak` при сбое) и опциональная проверка новой версии при старте (`check_updates`).
...
- `line_dead_after_sec` - через сколько секунд без keepalive линия считается «не живой» (по умолчанию 120). Влияет на поле `alive` в `/lines`, на `/health`, на авто-выбор линии при пустом `line` и на webhook-события `line_down`/`line_up`.
- `fail_threshold` - сколько ошибок отправки подряд на одной линии считать проблемой канала (по умолчанию 10): на webhook уходит `line_failing`, в `/status` линия помечается `suspect`; сбрасывается первой успешной отправкой (событие `line_recovered`).
- `clear_logs_on_start` - при старте переносить текущие логи bridge в одну копию `.prev` каждая (по умолчанию `true`): папка не зарастает, а лог прошлого запуска - в том числе упавшего - сохраняется.
- `check_updates` - фоновая проверка новой версии на GitHub при старте (по умолчанию `false`, bridge никуда не «звонит»).
...
Если `line` оставить пустой без MySQL, bridge выберет живую линию по кругу (round-robin, с учётом `default_lines`). Для production лучше указывать конкретную линию, например `Go1`.
...
Скачивайте asset `goip-bridge` из последнего релиза (сейчас `v0.5.0`). Не скачивайте `Source code` и не используйте `Code -> Download ZIP`.
...
Опубликованы релизы `v0.1.0`, `v0.2.0`, `v0.3.0`, `v0.3.1`, `v0.3.2` и `v0.4.0`. Последний релиз `v0.5.0` содержит готовый asset `goip-bridge` размером около 11 MB для Linux x86-64 / amd64.
